...
apply_custom_style()
apply_page_accent("#1565C0")

st.title("🔍 SQL Analytics")
st.caption("25 practice queries across beginner, intermediate, and advanced levels.")

# ============================================================
# Query bank: label -> SQL string
# ============================================================
QUERIES = {
    ...
    "Q9 : All-rounders (>1000 runs & >50 wickets)": """
        ...
        bowling_agg AS (
            ...
            FROM bowling_stats bo JOIN matches m ON bo.match_id = m.match_id
    ...
    """,
    "Q19 : Most consistent batsmen (std dev)": """
        SELECT p.full_name,
               ROUND(AVG(b.runs_scored), 2) AS avg_runs,
    ...
    """,
    "Q20 : Format-wise matches & averages (>=20 matches)": """
        WITH per_format AS (
    ...
    """,
    "Q24 : Best batting partnerships (pairs)": """
    ...
    """,
    "Q25 : Quarterly performance trend": """
        WITH quarterly AS (
...
# UI
# ============================================================
selected_question = st.selectbox("Choose a query", options=list(QUERIES.keys()))
